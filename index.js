...
let store = null;
let phoneNumber = global.pairNumber || null;
...
module.exports = async function startA17() {
  try {
    if (!global.auth || !global.sessionFile) {
      console.log("Invalid config - auth or sessionFile missing!");
      process.exit(1);
    }

    const usePairingCode = global.auth.toLowerCase() === "pairing";
    const useQR = global.auth.toLowerCase() === "qr";
    const sessionExists = checkSessionExists(global.sessionFile);
    const sessionValid = sessionExists && isSessionValid(global.sessionFile);

    console.log(chalk.blue(`Session Path: ${global.sessionFile}`));
    ...
    if (usePairingCode && needsAuth) {
      ...
      if (global.pairNumber && global.pairNumber.trim() && global.pairNumber.length > 6) {
        phoneNumber = global.pairNumber;
        console.log(chalk.green(`Using configured phone number: ${phoneNumber}`));
      }
      
      if (!phoneNumber) {
        console.log(chalk.yellow("No phone number found in config. Please enter your details:"));
    ...
    // Enhanced pairing code logic - Only run if session is invalid and pairing is needed
    if (usePairingCode && needsAuth && !A17.authState.creds.registered) {
      const cleanedNumber = phoneNumber.replace(/[^0-9]/g, "");
      setTimeout(async () => {
        try {
          const code = await A17.requestPairingCode(cleanedNumber);
    ...
    A17.ev.on("connection.update", async (update) => {
      ...
      // QR code display for QR mode - only if session doesn't exist
      ...
      if (connection === "close") {
        ...
        if (reason === DisconnectReason.badSession) {
          ...
          if (fs.existsSync(global.sessionFile)) {
            fs.rmSync(global.sessionFile, { recursive: true, force: true });
          }
        ...
        const userName = A17.user?.name || global.BotName;
        const ownNumber = A17.user?.id?.split(":")[0] || "unknown";
